...
- Default plugins + changelog, checkbox, cut, file, images, includes, links
- Plugin errors/warnings collected via `log.error/warn/info` callbacks
- `utils.ts` converts lint errors and plugin messages to VS Code diagnostics with precise ranges (link ranges, asset ranges, directive ranges, etc.)
