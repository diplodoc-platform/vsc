...
Сборки с настроенным адресом телеметрии отправляют события использования функций и категории ошибок в Яндекс через приёмник в Yandex Cloud. События содержат случайный постоянный ID установки, ID сессии, версии расширения и VS Code, ОС, время, выбранные действия и числовые счётчики. ID установки не связан с учётной записью пользователя.

Содержимое документов, пути к файлам, имена пользователей, тексты и стеки ошибок не передаются. Принимающая инфраструктура может записывать IP-адреса источников запросов. Чтобы отключить сбор, установите в VS Code `telemetry.telemetryLevel: off`; уровень `error` разрешает только категории ошибок. При отключении очередь очищается, текущие запросы отменяются. Сборки без настроенного адреса телеметрию не отправляют.
