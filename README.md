...
- **Anchor Validation** — warns (yellow underline) when a `#anchor` in a Markdown link points to a heading or `{#id}` that does not exist in the target file; works for cross-file links, same-file `#anchor` links, and links inside YAML block scalars; skips fenced code blocks; runs on open, save, and change (400 ms debounce)
